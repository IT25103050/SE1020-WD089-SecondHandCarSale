...
public class CarDAO {
    ...
    public CarDAO(ServletContext servletContext) {
        String path = servletContext.getRealPath("/WEB-INF/data/cars.txt");
        if (path == null) {
            System.err.println("[CarDAO] WARNING: ServletContext.getRealPath returned null. Falling back to relative path.");
            path = "src/main/webapp/WEB-INF/data/cars.txt";
        }
        this.filePath = path;
    }
    ...
    public void saveCar(Car car) {
        if (car == null) return;
        FileHandler.writeLine(filePath, car.toFileString());
    }
