...
public class AuthController {
    ...
    public String processRegister(@RequestParam("fullName") String fullName,
        ...
        String userId = IDGenerator.generateUserId(FILE_PATH);
        String hashedPass = PasswordUtil.hashPassword(password);
        String selectedRole = ValidationUtil.isEmpty(role) ? "Buyer" : role.trim();
        ...
        if ("Seller".equalsIgnoreCase(selectedRole)) {
        ...
        } else if ("Dealer".equalsIgnoreCase(selectedRole)) {
            ...
            newUser = new Admin(userId, fullName.trim(), email.trim(), hashedPass, phone.trim(), address.trim(), "ACTIVE", "APPROVED", createdAt);
        } else {
            newUser = new Buyer(userId, fullName.trim(), email.trim(), hashedPass, phone.trim(), address.trim(), "ACTIVE", approvalStatus, createdAt);
        }
